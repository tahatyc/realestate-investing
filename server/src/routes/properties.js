...
export function createPropertiesRouter({ database } = {}) {
  ...
  router.get('/', asyncHandler(async (req, res) => {
    ...
    const [properties, total] = await Promise.all([
      queryProperties({ ...filters, limit, offset }, database),
      countProperties(filters, database)
    ]);
  ...
  router.get('/:id', asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      priceHistory: await getPriceHistoryByPropertyId(property.id, database),
      leverageSettings: settings.leverage,
      strategies: await analyzeProperty(property, { database, settings })
    });
  }));
